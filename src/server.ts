...
import { z } from "zod";
import { calculateTax, getFilingDeadline, TAX_BRACKETS, FILING_STATUSES } from "./taxUtils.js";
...
server.registerTool(
  "calculateTax",
  {
    ...
    description: "Calculate federal income tax based on income and filing status",
    inputSchema: {
      income: z.number().describe("Annual gross income in USD"),
      filingStatus: z.enum(['single', 'married', 'head_of_household', 'qualifying_widow'])
      ...
      content: [
        {
          ...
          text: `Tax Calculation Results:\n\n` +
                ...
                `Tax Bracket: ${result.bracket.description} (${(result.bracket.rate * 100).toFixed(0)}%)`
        },
      ],
    };
  }
...
server.registerTool(
  "getFilingStatusInfo",
  {
    ...
    description: "Get information about different filing statuses and their standard deductions",
    inputSchema: {
      status: z.enum(['single', 'married', 'head_of_household', 'qualifying_widow']).optional()
    ...
    if (status) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Filing Status: ${filingStatus.description}\n` +
                  `Standard Deduction (2024): $${filingStatus.standardDeduction.toLocaleString()}\n\n` +
                  `Tax Brackets:\n${TAX_BRACKETS[status].map(bracket => 
                    `• $${bracket.minIncome.toLocaleString()} - ${bracket.maxIncome ? `$${bracket.maxIncome.toLocaleString()}` : 'unlimited'}: ${(bracket.rate * 100).toFixed(0)}%`
                  ).join('\n')}`
          },
        ],
      };
    } else {
      const allStatuses = Object.entries(FILING_STATUSES).map(([, value]) => 
        `• ${value.description}: $${value.standardDeduction.toLocaleString()}`
      ).join('\n');
...
server.registerTool(
  "getTaxAssistance",
  {
    ...
    inputSchema: {
      question: z.string().describe("The tax-related question or topic you need help with")
    }
  },
  async ({ question }) => {
    const commonTopics = [
      'deductions', 'credits', 'extensions', 'refunds', 'payment plans',
      'business expenses', 'home office', 'charitable donations', 'retirement accounts'
    ];
    ...
    if (lowerQuestion.includes('deduction') || lowerQuestion.includes('deduct')) {
    ...
    } else if (lowerQuestion.includes('credit') || lowerQuestion.includes('refund')) {
      response += "Common tax credits include:\n• Child Tax Credit\n• Earned Income Tax Credit (EITC)\n• American Opportunity Credit (education)\n• Lifetime Learning Credit\n• Child and Dependent Care Credit";
    } else if (lowerQuestion.includes('extension') || lowerQuestion.includes('deadline')) {
      response += "If you need more time to file:\n• File Form 4868 for an automatic 6-month extension\n• Extension must be filed by the original deadline\n• Extension gives you time to file, not to pay\n• You may still owe penalties and interest on unpaid taxes";
    } else if (lowerQuestion.includes('payment') || lowerQuestion.includes('owe')) {
      response += "Payment options if you owe taxes:\n• Installment Agreement (Form 9465)\n• Offer in Compromise\n• Currently Not Collectible status\n• Pay with credit card (fees apply)\n• Set up automatic payments";
    } else {
      response += "Please provide more specific details about your tax situation so I can give you the best guidance possible. Common topics I can help with include: " + commonTopics.join(', ');
    }
