...
Object.entries(FILING_STATUSES).forEach(([, status]) => {
  console.log(
    `  • ${
      status.description
    }: $${status.standardDeduction.toLocaleString()} standard deduction`
  );
});
console.log();
...
console.log(
  `  $100,000 income → ${singleBrackets.bracket.description} (${(
    singleBrackets.bracket.rate * 100
  ).toFixed(0)}%)`
);
...
console.log(
  `  $100,000 income → ${marriedBrackets.bracket.description} (${(
    marriedBrackets.bracket.rate * 100
  ).toFixed(0)}%)`
);
console.log();
...
console.log(
  `Single filer with $80,000: $${singleTax.taxAmount.toLocaleString()} tax`
);
console.log(
  `Married filing jointly with $80,000: $${marriedTax.taxAmount.toLocaleString()} tax`
);
...
console.log(
  "• Tax brackets are progressive (different rates for different income levels)"
);
