...
Federal Income Tax Withheld: $0
    ...
    const interestId = await processor.uploadDocument(
      "1099_int_2024.txt",
      interestContent,
      "1099"
    );
...
Federal Income Tax Withheld: $0
    ...
    const dividendId = await processor.uploadDocument(
      "1099_div_2024.txt",
      dividendContent,
      "1099"
    );
    ...
    const formats = ["json", "xml", "mail_ready"] as const;
    ...
    for (const format of formats) {
      ...
      console.log(
        `Summary: Tax Owed: $${result.summary.taxOwed.toLocaleString()}, Refund: $${result.summary.refundAmount.toLocaleString()}`
      );
      console.log(`Forms generated: ${result.forms.length}`);
      console.log(
        `First form content preview: ${result.forms[0].content.substring(
          0,
          100
        )}...\n`
      );
